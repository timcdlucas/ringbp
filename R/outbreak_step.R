  ...
  if(testing==TRUE) {
      ...
      prob_samples[, isolated_end := ifelse(vect_isTRUE(test), # If tested
                                            ifelse(vect_isTRUE(test_result), # If positive
                                              isolated_time[which(prob_samples$test_result == TRUE)] + 
                                                runif(sum(prob_samples$test_result == TRUE, na.rm = TRUE), min_isolation, max_isolation), # Stay in isolation for a random length
