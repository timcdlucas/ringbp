...
scenarios1 <- tidyr::expand_grid(
  ...
  filter((min_isolation == 7 & max_isolation == 14 & iso_adhere == 0.2) |
         (min_isolation == 4 & max_isolation == 7 & iso_adhere == 0.3)) 
...
saveRDS(sweep_results1, file = "data-raw/res_four_day_adhere.rds")

#if(!exists('sweep_results1'))  sweep_results1 <- readRDS(file = "data-raw/res_four_day_adhere.rds")
...
ggsave('inst/plots/four_days_vs_20pc_adherence.png')
